...
typeof :: Context -> Term -> Either TypeErrorTerm Type
...
typeof ctx ( TmApp t1 t2 ) = case ( typeof ctx t1, typeof ctx t2 ) of
  ( Right leftType@( TypeArrow type1 type2 ), Right type1' ) -> if type1 == type1'
    then Right type2
    else Left AppTypeError{leftTerm=t1, rightTerm=t2, leftType=Right leftType, rightType=Right type1'}
  ( e1, e2 ) -> Left AppTypeError{leftTerm=t1, rightTerm=t2, leftType=e1, rightType=e2}
